...
export default function DentalSurgeonPatients() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const emptyQuadrants = { upperLeft: "", upperRight: "", lowerLeft: "", lowerRight: "" };
    const body = { appointmentId: patient.appointmentId, patientAge: age ? Number(age) : undefined, patientGender: form.get("patientGender"), diagnosis: form.get("diagnosis"), complaints: form.get("complaints"), onExamination: form.get("onExamination"), medicalHistory: form.get("medicalHistory"), treatmentPlan: form.get("treatmentPlan"), investigations: form.get("investigations"), complaintQuadrants: emptyQuadrants, examinationQuadrants: emptyQuadrants, investigationQuadrants: emptyQuadrants, diagnosisQuadrants: emptyQuadrants, treatmentPlanQuadrants: emptyQuadrants, advice: form.get("advice"), followUpDate: form.get("followUpDate"), medicines: prescriptionMedicines };
    try {
  ...
  return <div>
    ...
    {patient && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4"><form onSubmit={submit} className="mx-auto my-4 w-full max-w-5xl rounded-lg bg-white shadow-xl"><header className="flex items-start justify-between border-b border-slate-200 px-6 py-4"><div><h3 className="text-lg font-semibold text-slate-900">New prescription</h3><p className="text-sm text-slate-500">{patient.name} · {patient.phone}</p></div><button type="button" title="Close" onClick={() => setPatient(null)} className="p-2 text-slate-500 hover:text-slate-900"><FiX size={20}/></button></header>
      <div className="space-y-6 p-6">{error && <p className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}
        <section><h4 className="mb-3 text-sm font-semibold text-slate-800">Patient and clinical details</h4><div className="grid gap-3 sm:grid-cols-2"><input name="patientAge" type="number" min="0" max="130" placeholder="Age" className="rounded-md border border-slate-300 px-3 py-2 text-sm"/><select name="patientGender" className="rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">Gender</option><option>Male</option><option>Female</option><option>Other</option></select><textarea name="complaints" placeholder="Chief complaints (C/C)" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" rows={2}/><textarea name="onExamination" placeholder="On examination (O/E)" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" rows={9}/><textarea name="medicalHistory" placeholder="Medical history (M/H)" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" rows={3}/><textarea name="investigations" placeholder="Investigations / tests" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" rows={2}/><textarea required name="diagnosis" placeholder="Diagnosis (D/D) *" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" rows={2}/><textarea name="treatmentPlan" placeholder="Treatment plan (Rx Plan)" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" rows={7}/></div></section>
        <section><div className="mb-3 flex items-center justify-between"><h4 className="text-sm font-semibold text-slate-800">Medicines</h4><button type="button" onClick={() => setMedicines((m) => [...m, emptyMedicine()])} className="inline-flex items-center gap-1 text-sm font-medium text-sky-700"><FiPlus/> Add medicine</button></div><div className="space-y-3">{medicines.map((m, i) => <div key={i} className="grid gap-2 rounded-md border border-slate-200 bg-slate-50 p-3 md:grid-cols-2 lg:grid-cols-8">
          <div className="min-w-0"><label className="mb-1 block text-xs font-medium text-slate-600">Medicine *</label><select required value={m.medicineOption} onChange={(e) => updateMedicineOption(i, e.target.value)} className="w-full min-w-0 rounded-md border border-slate-300 bg-white px-2 py-2 text-sm"><option value="" disabled>Select medicine</option>{MEDICINE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}<option value={OTHER_OPTION}>Others</option></select></div>
          <div className="min-w-0"><label className="mb-1 block text-xs font-medium text-slate-600">Name *</label><input required value={m.name} onChange={(e) => updateMedicine(i, "name", e.target.value)} placeholder="Medicine name" className="w-full min-w-0 rounded-md border border-slate-300 bg-white px-2 py-2 text-sm"/></div>
          ...
          {(["dosage","frequency","duration"] as const).map((field) => <div key={field} className="min-w-0"><label className="mb-1 block text-xs font-medium text-slate-600">{{dosage:"Dose *",frequency:"Frequency *",duration:"Duration *"}[field]}</label><input required value={m[field]} onChange={(e) => updateMedicine(i, field, e.target.value)} placeholder={({dosage:"Dose",frequency:"Frequency",duration:"Duration"})[field]} className="w-full min-w-0 rounded-md border border-slate-300 bg-white px-2 py-2 text-sm"/></div>)}
          <div className="min-w-0"><label className="mb-1 block text-xs font-medium text-slate-600">Instructions</label><select value={m.instructions} onChange={(e) => updateMedicine(i, "instructions", e.target.value)} className="w-full min-w-0 rounded-md border border-slate-300 bg-white px-2 py-2 text-sm"><option>Before Meal</option><option>After Meal</option></select></div>
